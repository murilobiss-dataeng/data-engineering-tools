...
| `CHAT_ID` | **Prioridade:** ID do canal (ex.: `120363405814099508@newsletter`). Quando definido, o bot usa só este e ignora `CHAT_IDS`. |
| `CHAT_IDS` | **Fallback:** quando `CHAT_ID` não está definido, usa este — URL completa (ex.: `https://whatsapp.com/channel/CODIGO`) ou vários IDs/URLs separados por vírgula. |
...
4. Coloque no `.env`: `CHAT_ID=12345678901234567@newsletter` (canal) ou, sem CHAT_ID, `CHAT_IDS=120363XXXXXXXX@g.us` (grupo).
...
- **`CHAT_ID`** (recomendado) — ID do canal (ex.: `120363405814099508@newsletter`). Quando definido, o bot usa só este. **`CHAT_IDS`** — fallback: URL do canal ou ID de grupo (`XXX@g.us`). A conta que escaneou o QR deve ser **admin do canal**.
